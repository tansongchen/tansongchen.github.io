...
export default function ({ data }: PageProps<Queries.ArticleQuery>) {
  ...
  const { fileAbsolutePath, frontmatter, html } = data.markdownRemark || { fileAbsolutePath: "", frontmatter: {}, html: "" };
  const { title, date, tags } = frontmatter || {};
  const slug = basename(fileAbsolutePath || "");
  return <Layout slug={`articles/${slug}`}>
    ...
    <main className="section">
      <div className="container is-max-desktop content" dangerouslySetInnerHTML={{ __html: html || "" }}>
      </div>
    </main>

...
class Form extends Component<FormProps, FormState> {
  ...
  render() {
    ...
    const displayOrderType = (o: OrderType) =>
    ...
    const mealOptions =
      this.state.date.getDay() === 0 || this.state.date.getDay() === 6
    ...
    const newShouldSelect = (o: OrderType) =>
      o === OrderType.select ? true : false;
    let summary: string[] = [];
        ...
        <div className="level">
          <div className="level-left">
            ...
            <Dropdown<OrderType>
              ...
              callback={(x) => {
                changeOrderType(x);
                this.props.changeSelect(newShouldSelect(x));
              }}
              current={this.state.ordertype}
              display={displayOrderType}
            />
          </div>
          <div className="level-right">
            ...
            <Dropdown<MealType>
              options={mealOptions}
              callback={changeMealType}
              current={this.state.mealtype}
              display={displayMealType}
            />
            <div style={{ padding: ".5rem", display: "inline-flex" }}>品尝</div>
          </div>
        </div>
        {this.props.shouldSelect ? (
          <ul className="block">
            {summary.map((x) => (
              <SummaryItem key={x} name={x} update={this.props.update} />
            ))}
          </ul>
        ) : (
          <div></div>
...
class Order extends Component<OrderProps, OrderState> {
  ...
  onSubmitOrder = async (form: FormState) => {
    ...
    const summary: string[] = [...this.props.selected.entries()]
    ...
    return (
      <section className="section">
        <div className="container is-max-desktop">
          <Form
            submit={this.onSubmitOrder}
            submitting={submitting}
            selected={this.props.selected}
            update={this.props.update}
            shouldSelect={this.props.shouldSelect}

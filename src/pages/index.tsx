...
const Introduction = () => (
  <section
    ...
    style={{
      backgroundImage:
        "linear-gradient(to bottom, rgba(200,240,255,0.5), rgba(255,255,255,0.5))",
    }}
  >
    <div className="container is-max-widescreen">
      <div className="columns is-vcentered">
        ...
        <div
          ...
          style={{ padding: "5%", fontSize: "125%" }}
        >
          <p>欢迎来到我的个人网站！我是谭淞宸。</p>
          <p>
            1974 年，Internet 一词第一次出现在 RFC
            提案中，标志着世界各地的小型网络走向互联。1987 年 9 月 14
            日，北京计算机应用技术研究所发出了中国第一封电子邮件：「
            <i>Across the Great Wall we can reach every corner in the world.</i>
            」。几十年后的今天，商业化的互联网中我们接收到的绝大多数信息来自科技公司的整合与推送，但是我们仍然可以以个人网站的形式保留并发出自己独立的声音。
          </p>
          <p>
            我自诩为一位创造者，凡是将人类的心智外化于物、创造出本不属于这个世界的实体从而实现自我的活动都令我兴致勃勃，写文章、敲代码、玩音乐、拍视频……我想自古以来的文人墨客都要给自己的书斋、画室起个名号，我也不能免俗地依志趣把这个网站称作「
            <span>{title}</span>」。您可以在导航栏中发现各个版块。祝您浏览愉快！
          </p>
          <p>
...
const Summary = ({ slug, image, name, description }: Art) => (
...
const EducationItem = ({
  ...
  <article className="section container is-max-desktop">
    <div
    ...
    >
      <div className="column" style={{ padding: 0 }}>
        <a href={institutionUrl}>{photo}</a>
      </div>
      <div className="column content" style={{ padding: "2rem" }}>
        <h3 className="has-text-centered">{institutionName}</h3>
...
const mit: EducationProps = {
  ...
  duration: "2021 - 2023",
  description:
    "作为计算科学与工程中心开设的研究型硕士项目，我在这里接受高质量的计算相关课程的训练，并与来自全校各系的教授和学生共同以跨学科的方式探讨计算问题。",
};

const ucla: EducationProps = {
  photo: (
    <StaticImage
      src="../images/education/ucla.jpg"
      alt="ucla"
      width={480}
      height={360}
    />
  ...
  duration: "2019 - 2019",
  description:
    "大三时，我赴加州大学洛杉矶分校进行一学期的交换。沐浴在加州的暖阳中，我与校园吉祥物加州棕熊 Bruin 共同感受校园的学术氛围。",
};

const pku: EducationProps = {
  photo: (
    <StaticImage
      src="../images/education/pku.jpg"
      alt="pku"
      width={480}
      height={360}
    />
  ...
  degrees: [
    ["理学学士，化学", "https://chem.pku.edu.cn"],
    ["理学学士，物理", "https://phy.pku.edu.cn"],
  ],
  location: "北京市",
  duration: "2017 - 2021",
  description:
    "未名湖畔、博雅塔下，细推物理须行乐，何用浮名绊此身。四年间，我在化学和物理双学位中取得 GPA 3.86 / 4，并在毕业时获未名学士荣誉。",
};

const rdfz: EducationProps = {
  photo: (
    <StaticImage
      src="../images/education/rdfz.jpg"
      alt="rdfz"
      width={480}
      height={360}
    />
  ...
  duration: "2011 - 2017",
  description:
    "海淀黄庄的路口承载了我快乐的少年时光；我曾随合唱团出国交流演出，也曾参与食虫植物温室的建设。高一时在第 29 届化学奥林匹克决赛中取得金牌，并提前一年毕业。",
};
...
const Preface = ({ title, quote, description }: PrefaceProps) => (
...
const ExperienceItem = ({
  ...
  <article className="timeline-item">
    <div className="timeline-marker is-icon">
      {isCurrent ? (
        <FaPlayCircle className="icon is-info" />
      ) : (
        <FaCheckCircle className="icon is-info" />
      )}
    </div>
    <div className="timeline-content" style={{ width: "100%" }}>
      <div className="columns">
        <div className="column content" style={{ padding: "1rem 2rem" }}>
          ...
          <p>
            <FaCalendar className="icon is-small" />
            &nbsp;&nbsp;{duration}
          </p>
          <p>
            <FaMapMarker className="icon is-small" />
            &nbsp;&nbsp;{location}
          </p>
          {description}
        </div>
...
const nvidia: ExperienceData = {
  ...
  description: (
    <p>
      在 NVIDIA
      深度学习编译器团队，我探究了深度神经网络编译过程中的层融合（fusion）优化，提高了自动调优的效果并提升了编译效率。
    </p>
  ),
...
const julia: ExperienceData = {
  ...
  description: (
    <p>
      在 Julia 实验室，我们利用 Julia 语言的一系列编译基础设施（元编程、IR
      反射和 LLVM
      代码生成）研究如何将高级语言表达的抽象语义有效地编译为并行计算、异构计算等高性能场景下的机器代码。
    </p>
  ),
...
const dptech: ExperienceData = {
  ...
  description: (
    <p>
      在深势科技，我们利用 DeePKS
      方法构建参数化的密度泛函模型，并设计有效的训练方案来系统改善这样的模型；我们还将模型部署到
      ABACUS 等开源密度泛函软件上实现高效的推理。
    </p>
  ),
...
const binz: ExperienceData = {
  ...
  description: (
    <p>
      在 Zhang 组，我们利用深度生成模型对 DNA
      体系进行粗粒化建模，并利用噪声对比学习进行优化，相较于传统的
      force-matching 方法在统计量的准确性上得到了较大的提升。
    </p>
  ),
...
const thg: ExperienceData = {
  ...
  description: (
    <Fragment>
      <p>
        在 Head-Gordon 组，我们发展了一种利用扩展 Langrange
        量对经典电荷平衡模型进行近似模拟的方法，并基于 MPI
        高效地在大规模分子体系中实现了这一方法。该方法已加入了开源分子动力学软件
        LAMMPS。请参考以下两篇发表的文章：
      </p>
      <ul>
        <li>
          <a href="https://doi.org/10.1021/acs.jctc.1c00118">
            I. Leven, H. Hao, <strong>S. Tan</strong>, ... T. Head‐Gordon,{" "}
            <em>J. Chem. Theory Comput.</em> <b>2021.</b>
          </a>
        </li>
        <li>
          <a href="https://doi.org/10.1021/acs.jctc.0c00514">
            <strong>S. Tan</strong>, I. Leven, D. An, L. Lin, T. Head‐Gordon,{" "}
            <em>J. Chem. Theory Comput.</em> <b>2020.</b>
          </a>
        </li>
      </ul>
    </Fragment>
...
const DateTag = ({ date }: { date: string }) => (
  <header className="timeline-header">
    <span className="tag is-medium is-info" style={{ minWidth: "4rem" }}>
      {date}
    </span>
  </header>
);

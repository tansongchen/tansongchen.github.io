...
const MusicView = ({ name, date, opus, number, category }: Music) => (
  <article className="container content is-max-desktop" key={name}>
    <h2>
...
const Display = ({ nodes }: { nodes: Music[] }) => (
  <section className="section">{nodes.map(MusicView)}</section>
);

export default function ({ data }: PageProps<Queries.MusicsQuery>) {
  const nodes: Music[] = data.notionDatabase!.childrenNotionPage!.map(
    (page) => {

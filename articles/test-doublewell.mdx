...
date: 2019-07-26
tags: [物理, 化学]
cover: https://cdn.jsdelivr.net/gh/tansongchen/images@master/double-well.webp
...
>
>
> 现设系统经历了一个过程并改变了它的 Hamilton 量，则在这个过程中 Hamilton 量可以写作 *H*(**x**, *λ*)，其中 *λ* = 0 对应初态，*λ* = 1 对应末态。在这个过程中 *λ* 随时间的变化关系记作 *λ*(*t*)。
>
...
$$ 除去普通的动能项，剩下的势能项由 *λ* = 0 时的一个双势阱 *q*4 − 16*q*2 变成 *λ* = 1 时的一个单势阱 *q*4。

![http://ww3.sinaimg.cn/large/006tNc79ly1g5i1ftdh49j31gz0qowf0.jpg](http://ww3.sinaimg.cn/large/006tNc79ly1g5i1ftdh49j31gz0qowf0.jpg)
...
在 [7 月 19 日的文章](https://tansongchen.com/symplecticity/) 中，我们总结了在 Hamilton 量不含时的情况下，辛算法的一般构建方式。不过上面的讨论要求我们处理一个 Hamilton 量随标度参量变化，因而也就随时间变化的情况：*H* = *H*(*p*, *q*, *λ*(*t*))。我们接下来要说明的是：对于 Hamilton 量含时的情况，尽管 Hamilton 量不守恒，但原有的辛算法仍然保持某种部分的（或者说准的）辛性。

先考虑一般的情况，即 ℝ2*n* 上的正则方程：

我们引入虚拟位置 *qn* + 1 = *t* 以将 Hamilton 量表示成不含时间的形式 *H*(*p*1, …, *pn*, *q*1, …, *qn*, *qn* + 1)，以及和它共轭的虚拟动量 *pn* + 1 =  − *H*，还有扩充的 Hamilton 量 *K* = *H* + *pn* + 1。原有的正则方程仍然保持其形式： $$
\begin{aligned}
...
\end{aligned}
$$ 可见这是自洽的。虽然 *qn* + 1 在数值演化上完全等价于 *t*，但从形式上看 *K* 只含有 2*n* + 2 个相空间坐标而不含时，从而可以在这个 ℝ2*n* + 2 空间上设计辛算法。
...
H_3&=\frac{s\pi^2}{2M}
\end{aligned}
$$ 我们发现，如果将 *V*(*q*, *Q*) 替换 *V*(*q*) 放在 *H*2 中，那么 *H*2 仍然只含广义位置而不含广义动量，因此仍然可以精确求出时间演化算符；另外如果将 *P* 加到 *H*1 中，那么 *H*1 只含广义动量 *p*, *P* 和广义位置 *s*，它们互相不共轭，因此也可以精确求出时间演化算符。也即：*K* = *K*1 + *K*2 + *K*3，其中 $$
\begin{aligned}
K_1&=P+s\left[\frac{p^2}{2s^2}+\ln s-H_{0}\right]\\
K_2&=sV(q,Q)\\
K_3&=\frac{s\pi^2}{2M}
\end{aligned}
$$ 对应的辛差分格式仍然适用：**x***n* + 1 = *eΔtH*3/2*eΔtH*2/2*eΔtH*1*eΔtH*2/2*eΔtH*3/2**x***n*，这里 **x** 是 6 维向量。我们具体将这个格式写出来（**高能预警**）：
...
为了定量刻画不充分采样产生的误差，我们需要一点统计学的分析。为简便起见，移动能量零点使初态 *F* = 0，这样 Jarzynski 等式变为 *F* =  − ln ⟨*e* − *W*⟩ 从这个等式出发，我们可以将基于 *N* 个样本的某种统计平均称为一个估值（estimator） *FN*，例如最简单的估值是 $$
F_N=-\ln\left(\frac1N\sum_{i=1}^Ne^{- W_i}\right)
$$

---

理想的估值方案需要在尽可能少的计算时间内获得尽可能精确的自由能，为此我们定义均方误差： *ε*2[*FN*] = ⟨(*FN*−*F*)2⟩ 这个均方误差可以分为两部分： ⟨(*FN*−*F*)2⟩ = ⟨(*FN*−⟨*FN*⟩)2⟩ + (⟨*FN*⟩−*F*)2 前一部分是估值自身的方差（随机误差），记作 *σ*2[*FN*]；后一部分是估值期望与真值的偏差（系统误差），记作 *b*2[*FN*]，总而言之 *ε*2[*FN*] = *σ*2[*FN*] + *b*2[*FN*]

## 直接指数平均是有偏估计

上述最简单的估值方案表现如何？首先考虑它的系统误差 *b*2，我们引入记号 $$
X=e^{-W},X_N=\frac1N\sum_{i=1}^Ne^{-W_i}
$$ 也即 *FN* =  − ln *XN*，那么这个估值的期望是 ⟨*FN*⟩ = ⟨ − ln *XN*⟩

由 Jensen’s （琴生）不等式，⟨ln *XN*⟩ < ln ⟨*XN*⟩，而 ⟨*XN*⟩ = ⟨*X*⟩ = *e* − *F*，故有 ⟨*FN*⟩ ≥  − ln *e* − *F* = *F*

这说明我们总是高估真实的自由能变。

---

高估了多少？我们记随机变量 *X* 的期望为 *m*，方差为 *s*2，⟨*X*⟩ = *e* − *F* = *m*，可以将 ln ( ⋅ ) 在 *XN*/*m* 处展开： $$
...
$$ 其中 *YN* = *XN*/*m* − 1。 − ln *m* 是真值，所以系统误差是后面的级数部分，又因为 ⟨*YN*⟩ = 0，所以系统误差的主导项是 $$
...
$$ 鉴于 *bN*2 = *O*(*N* − 2)，*σN*2 似乎是 *εN*2 的主要来源，所以我们将只考虑 *εN*2 ≈ *σN*2。

---

又由于 *X* = *e* − *W*，所以 *εN*2 可以化简为（*Wd* 是耗散功 *W* − *F*） $$
\varepsilon_N^2\approx \frac{\langle (e^{-W_d}-1)^2\rangle}N
$$ 换言之，获得准确度为 *kT* 的数据所需的采样数目为 *N* ≈ ⟨(*e* − *Wd* − 1)2⟩ 上篇文章中提到的绝大多数计算方法基于在一定条件下最小化 ⟨(*e* − *Wd* − 1)2⟩。
...
- 每个轨迹在 10 秒内以 10 步长演化，每次采样 10 个轨迹，

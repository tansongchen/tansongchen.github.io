...
date: 2019-12-04
tags: [物理, 数学]
cover: https://cdn.jsdelivr.net/gh/tansongchen/images@master/langevin-equations.webp
abstract: 在线性极限下证明了 Langevin 运动方程给出了系统的正则分布。
---

## 统计力学与分子动力学

经典统计力学告诉我们，正则系综中系统在 2*n* 维相空间的坐标 **x** = (**q**, **p**) 应该符合正则分布，即系统处在坐标 **x** 附近的概率应该正比于 Boltzmann 因子 *e* − *βH*(**x**)。

根据统计力学的基本假设，任何宏观物理量期望值都可以表述为以 Boltzmann 因子为权重的加权平均： ⟨*A*⟩ = ∫*d***x***A*(**x**)*e* − *βH*(**x**) 也就是说，如果我们通过计算模拟的方法，能按 *e* − *βH*(**x**) 对系统进行采样，在每次采样中计算物理量 *A* 并平均，就能得到正确的期望值。

**分子动力学**提供了这样的采样方法，它假定系统在长时间的演化过程中，在扩展的微正则系综意义下等概率地经过每一个微观态，从而在正则系综中以 *e* − *βH*(**x**) 为权重地经过每一个微观态。
...
\end{aligned}
$$ 最终的结果为 **x**(*t*) = *e***K***t*[**x**(0)+∫0*te* − **K***s***b***ds*] 由于 **x**(*t*) 的积分中含有随机项，因而它是一个随机变量；且随机过程的基本知识告诉我们它一定是一个二元正则分布的随机变量。二元正则分布完全由均值向量和协方差矩阵描述，所以我们只需要求出 $$
\begin{aligned}
...
$$ 因而均值即为 **μ**(*t*) = *e***K***t***x**(0) 从而协方差矩阵为（请左右滑动查看完整公式） $$
\begin{aligned}
&\boldsymbol\Sigma(t)\\
...
$$ 可见当 *γ* > 2 时，该矩阵本征值全部为负，那么 *e***K***t* 在 *t* → ∞ 时趋于零矩阵。因而 **μ**(∞) = **0** 而 **Σ**(*t*) 的长时极限则需要我们进行一番~~痛苦的~~计算，但最终我们可以得到 **Σ**(∞) = **1**/*β* 其中 **1** 为单位矩阵。现在协方差矩阵为对角阵，说明两个变量 *q*, *p* 的相关系数为 0，其分布密度函数由两个一元正态分布的乘积给出： $$
\begin{aligned}
...
\end{aligned}
$$ 注意：系统的 Hamilton 量为 *H* = (*q*2 + *p*2)/2，因而我们算出的分布密度函数恰为 *f*(*q*, *p*) ∝ *e* − *βH* 这说明，若我们在演化充分长的时间之后对运动轨迹采样，我们就能得到正则分布。我们得到结论：

...
由于只有算符的期望值是可观测量，所以只要得到正确的期望值，我们*可以对算符进行任意修改*。对于随时间演化的系统，我们因此可以将时间演化的效果从波函数完全转移到算符上。换句话说，如果系统的时间演化可以用 $|\Psi(t)⟩ = U(t)|\Psi(0)⟩$ 表示，那么我们将 Heisenberg 绘景下的算符 $O(t)$ 写成 $U^\dagger OU$ 就能包括时间演化的效果。特别对于 Hamilton 量不含时的系统，时间演化算符 $U=e^{-iHt}$。
...
其中，数字 1, 2, ..., 7 指代的是不同的位置和时间，即 $1\equiv (r_1, t_1)$。不幸的是，如果我们希望求解这样的问题，那将会比原来的多体问题更复杂！
